    ...
    public void purchaseProduct() {
        ...
        for (int i = 0; i < numberOfItems; i++) {
            ...
            if(selectedProduct.getStock() >= quantity){
                ...
                pendingTransactions.add(new Transactions(Login.loggedInCustomerName, selectedProduct.getName(), quantity, selectedProduct.getPrice()));
            } else {
        ...
        if(paymentType.equals("cash")){
            ...
            if(payment >= total) {
                ...
                float shortfall = total - payment;
                System.out.printf("Insufficient funds. You are short by: $%.2f\n", shortfall);
            }
            ...
            System.out.print("Expiration data (MM/YY): ");
            String expireData = scanner.nextLine();

            System.out.print("CCV: ");
            int ccv = scanner.nextInt();

            if(String.valueOf(cardNumber).length() >= 4 && !cardHolder.isEmpty() && !expireData.isEmpty() && ccv >= 100 && ccv <= 999){
        ...
        if(paymentSuccessful){
            for(Transactions t : pendingTransactions){
                t.save();
            }
